    ...
    <div class="container mx-auto px-4">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-4xl lg:text-5xl font-bold text-slate-900 mb-3">
                @if($selectedExtra)
                    Dokumentasi: {{ $extracurriculars->firstWhere('slug', $selectedExtra)?->name }}
                @else
            ...
            <p class="text-slate-600 max-w-2xl mx-auto">
                @if($selectedExtra)
                    Kumpulan foto dan dokumentasi kegiatan {{ $extracurriculars->firstWhere('slug', $selectedExtra)?->name }}
                @else
        ...
        <div class="max-w-4xl mx-auto">
                <!-- Search -->
                <form action="{{ route('galleries.index') }}" method="GET" class="mb-4">
                    @if($selectedExtra)
                        <input type="hidden" name="extracurricular" value="{{ $selectedExtra }}">
                    @endif
                    <div class="relative">
                        <input type="text" 
                               name="q" 
                ...
                <div class="flex gap-2 flex-wrap justify-center">
                    <a href="{{ route('galleries.index') }}" 
                       class="px-4 py-2 rounded-full text-sm font-medium transition-colors {{ !$selectedExtra ? 'bg-primary-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                        Semua
                    </a>
                    @foreach($extracurriculars as $extra)
                        <a href="{{ route('galleries.index', ['extracurricular' => $extra->slug]) }}" 
                           class="px-4 py-2 rounded-full text-sm font-medium transition-colors {{ $selectedExtra == $extra->slug ? 'bg-primary-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                            {{ $extra->name }}
                        </a>
                    @endforeach
                </div>
                ...
                @if($selectedExtra || $search)
                    <div class="text-center mt-4 text-sm text-slate-600">
                        ...
                        @endif
                        @if($selectedExtra)
                            <strong>{{ $extracurriculars->firstWhere('slug', $selectedExtra)?->name }}</strong>
                        @endif
                        · 
                        <a href="{{ route('galleries.index') }}" class="text-primary-600 hover:text-primary-700 font-medium">
                            Hapus filter
                        </a>
                    </div>
                @endif
    ...
    <div class="container mx-auto px-4">
        @if($galleries->count())
            <div class="grid md:grid-cols-4 gap-6">
                @foreach($galleries as $gallery)
                    <a href="/galleries/{{ $gallery->slug }}" class="group bg-white border border-slate-100 rounded-2xl overflow-hidden hover:shadow-xl hover:border-sky-200 transition-all duration-300">
        ...
        @else
            <div class="text-center py-16">
                ...
                @if($search || $selectedExtra)
                    <a href="{{ route('galleries.index') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-primary-600 text-white rounded-full font-semibold hover:bg-primary-700 transition-colors">
                        Lihat Semua Galeri
                    </a>
                @endif
            </div>
