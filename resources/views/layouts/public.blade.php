    ...
    <div class="min-h-screen flex flex-col">
        <header class="bg-white/90 backdrop-blur border-b border-slate-100 sticky top-0 z-30">
            <div class="container mx-auto px-4 py-4 flex items-center justify-between">
                ...
                <nav class="hidden md:flex items-center gap-6" aria-label="Main navigation">
                        ...
                        <div class="absolute top-full left-0 mt-2 w-64 bg-white rounded-xl shadow-2xl border border-slate-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50" role="menu">
                            <div class="py-2">
                                @foreach($navigation ?? [] as $nav)
                                    <a href="/page/{{ $nav['slug'] }}" class="block px-4 py-3 text-sm text-slate-700 hover:bg-sky-50 hover:text-sky-600 transition-colors border-b border-slate-100 last:border-b-0 {{ request()->is('page/'.$nav['slug']) ? 'bg-sky-50 text-sky-600' : '' }}" role="menuitem">
                                        {{ $nav['title'] }}
                                    </a>
                                @endforeach
                            </div>
                    ...
                    <!-- Berita Menu Button -->
                    <button id="beritaMenuBtn" class="{{ request()->is('posts*') ? 'text-sky-600 font-semibold bg-sky-50' : 'text-slate-600 hover:text-sky-600 hover:bg-slate-100' }} text-sm flex items-center gap-2 px-3 py-2 rounded-md transition-all duration-200" aria-haspopup="true" aria-expanded="false">
                        Berita
                    ...
                    <button id="ekstraMenuBtn" class="{{ request()->is('ekstrakurikuler*') ? 'text-sky-600 font-semibold bg-sky-50' : 'text-slate-600 hover:text-sky-600 hover:bg-slate-100' }} text-sm flex items-center gap-2 px-3 py-2 rounded-md transition-all duration-200" aria-haspopup="true" aria-expanded="false">
                        Ekstrakurikuler
        ...
        <div id="mobileMenuSidebar" class="fixed inset-y-0 right-0 w-64 bg-white shadow-2xl z-50 transform translate-x-full transition-transform duration-300 overflow-y-auto md:hidden" role="navigation" aria-label="Mobile menu">
            ...
            <div class="p-4 flex flex-col gap-2">
                <div class="mb-2">
                    ...
                    @foreach($navigation ?? [] as $nav)
                        <a href="/page/{{ $nav['slug'] }}" class="block px-3 py-2 text-sm rounded-md {{ request()->is('page/'.$nav['slug']) ? 'bg-sky-50 text-sky-600 font-medium' : 'text-slate-600 hover:bg-slate-50' }}">
                            {{ $nav['title'] }}
                        </a>
                    @endforeach
                </div>
        ...
        <div id="beritaMegaMenu" class="fixed w-[1000px] max-w-[calc(100vw-2rem)] bg-white rounded-xl shadow-2xl border border-slate-200 opacity-0 invisible transition-all duration-300 z-50 pointer-events-none" role="menu">
            <div class="p-6">
                ...
                @if(isset($recentPosts) && is_countable($recentPosts) && count($recentPosts) > 0)
                    <div class="overflow-x-auto scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100 pb-2">
                        <div class="flex gap-4" style="width: max-content;">
                            @foreach($recentPosts as $post)
                                <a href="/posts/{{ $post->slug }}" class="block w-64 bg-white border border-slate-100 rounded-lg overflow-hidden hover:border-sky-300 hover:shadow-lg transition-all duration-200 shrink-0 group/card">
        ...
        <div id="ekstraMegaMenu" class="fixed w-[1000px] max-w-[calc(100vw-2rem)] bg-white rounded-xl shadow-2xl border border-slate-200 opacity-0 invisible transition-all duration-300 z-50 pointer-events-none" role="menu">
            <div class="p-6">
                ...
                {{-- Dynamic ekstrakurikuler carousel --}}
                @if(isset($extracurriculars) && is_countable($extracurriculars) && count($extracurriculars) > 0)
                    <div class="overflow-x-auto scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100 pb-2">
                        <div class="flex gap-4" style="width: max-content;">
                            @foreach($extracurriculars as $ekstra)
                                ...
                                <a href="{{ route('galleries.index', ['extracurricular' => $ekstra->slug]) }}" class="block w-64 bg-white border border-slate-100 rounded-lg overflow-hidden hover:border-sky-300 hover:shadow-lg transition-all duration-200 shrink-0 group/card">
                                    @if($thumbnail)
                                        <div class="relative h-40 overflow-hidden bg-slate-100">
                                            <img src="{{ $thumbnail }}" 
                                                 alt="{{ $ekstra->name }}" 
                                    ...
                                    <div class="p-4">
                                        ...
                                        @if($ekstra->galleries->count() > 0)
                                            <div class="flex items-center text-xs text-slate-500">
                                                <svg class="w-3.5 h-3.5 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                                </svg>
                                                {{ $ekstra->galleries->count() }} Galeri
                                            </div>
                                        @endif
                                    </div>
                                </a>
