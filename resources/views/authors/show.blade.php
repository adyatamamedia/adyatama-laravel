        ...
        <div class="max-w-5xl mx-auto">
            ...
            <nav class="mb-6 flex justify-center">
                <ol class="flex items-center gap-2 text-sm text-slate-600">
                    ...
                    <li>
                        <a href="{{ route('authors.index') }}" class="hover:text-primary-600 transition-colors">Penulis</a>
                    </li>
                    <li>
                        <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                    ...
                    <li class="font-medium text-slate-900">{{ $author->fullname }}</li>
                </ol>
            </nav>
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-slate-200 p-8">
                <div class="flex flex-col md:flex-row items-center md:items-start gap-6">
                    ...
                    <div class="flex-1 text-center md:text-left">
                        <h1 class="text-3xl font-bold text-slate-900 mb-2">{{ $author->fullname }}</h1>
                        
                        @if($author->username)
                            <p class="text-slate-600 mb-4">{{ '@' . $author->username }}</p>
                        @endif

                        @if($author->email)
                            <a href="mailto:{{ $author->email }}" class="inline-flex items-center gap-2 text-sm text-slate-600 hover:text-primary-600 transition-colors mb-4">
                        ...
                        <div class="flex items-center gap-6 justify-center md:justify-start mt-6">
                            <div class="text-center">
                                <div class="text-2xl font-bold text-primary-600">{{ number_format($posts->total()) }}</div>
                                <div class="text-sm text-slate-600">Artikel</div>
                            </div>
                            <div class="w-px h-10 bg-slate-200"></div>
                            <div class="text-center">
                                <div class="text-2xl font-bold text-primary-600">{{ number_format($announcements->total()) }}</div>
                                <div class="text-sm text-slate-600">Pengumuman</div>
                            </div>
                            <div class="w-px h-10 bg-slate-200"></div>
                            <div class="text-center">
                                <div class="text-2xl font-bold text-primary-600">{{ number_format($galleries->total()) }}</div>
                        ...
                        @foreach($posts as $post)
                            <article class="group bg-white border border-slate-200 rounded-xl overflow-hidden hover:shadow-xl hover:border-primary-300 transition-all duration-300">
                                @if($post->featured_image_url)
                                    <a href="{{ route('posts.show', $post->slug) }}" class="block relative h-48 overflow-hidden bg-slate-100">
                                ...
                                <div class="p-5">
                                    ...
                                    <!-- Read More -->
                                    <a href="{{ route('posts.show', $post->slug) }}" class="inline-flex items-center gap-1 text-sm text-primary-600 font-medium hover:gap-2 transition-all">
                                        Baca Selengkapnya
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                                        </svg>
                        ...
                        @foreach($announcements as $announcement)
                            <article class="group bg-white border border-slate-200 rounded-xl overflow-hidden hover:shadow-xl hover:border-primary-300 transition-all duration-300">
                                @if($announcement->featured_image_url)
                                    <a href="{{ route('posts.show', $announcement->slug) }}" class="block relative h-48 overflow-hidden bg-slate-100">
                                ...
                                <div class="p-5">
                                    ...
                                    <!-- Title -->
                                    <h3 class="font-bold text-lg text-slate-900 mb-2 line-clamp-2 group-hover:text-primary-600 transition-colors">
                                        <a href="{{ route('posts.show', $announcement->slug) }}">
                                            {{ $announcement->title }}
                                        </a>
                                    </h3>
                                    ...
                                    <!-- Read More -->
                                    <a href="{{ route('posts.show', $announcement->slug) }}" class="inline-flex items-center gap-1 text-sm text-primary-600 font-medium hover:gap-2 transition-all">
                                        Baca Selengkapnya
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                                        </svg>
                ...
                @if($galleries->count() > 0)
                    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                        @foreach($galleries as $gallery)
                            <a href="{{ route('galleries.show', $gallery->slug) }}" class="group block bg-white border border-slate-200 rounded-xl overflow-hidden hover:shadow-xl hover:border-primary-300 transition-all duration-300">
                                <!-- Gallery Cover -->
                                <div class="relative h-48 overflow-hidden bg-slate-100">
                                    @if($gallery->items->count() > 0 && $gallery->items->first()->image_url)
                                        <img src="{{ $gallery->items->first()->image_url }}" 
                                             alt="{{ $gallery->title }}" 
                                             class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                                    @else
                                ...
                                <div class="p-5">
                                    ...
                                    <div class="flex items-center gap-3 text-xs text-slate-500">
                                        <span class="flex items-center gap-1">
                                            ...
                                            {{ number_format($gallery->view_count ?? 0) }}
                                        </span>
                                    </div>
                                </div>
                            </a>
